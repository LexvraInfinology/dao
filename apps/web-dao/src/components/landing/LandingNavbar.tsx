...
export const LandingNavbar: React.FC = () => {
  ...
  // If already connected, "Launch App" goes straight to /dao.
  // If not, clicking opens the wallet modal first.
  const handleLaunchApp = (e: React.MouseEvent) => {
    if (!wallet.isConnected) {
      e.preventDefault();
      setWalletModalOpen(true);
    }
    // If connected, let the Link navigate normally to /dao
  };
          ...
          <div className="flex items-center justify-between h-12 gap-2">
            ...
            <div className="hidden lg:flex items-center">
              <Link
                ...
                className="px-5 py-2.5 rounded-full font-semibold text-[13px] text-white bg-[#0B132B] hover:bg-[#1E293B] shadow-sm transition-all duration-200 flex items-center gap-1.5"
              >
                <span>{wallet.isConnected ? 'Go to Dashboard' : 'Launch App'}</span>
            ...
            <div className="flex lg:hidden items-center gap-1.5 sm:gap-2.5 shrink-0">
              ...
              >
                <span>{wallet.isConnected ? 'Dashboard' : 'Launch App'}</span>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white/95 backdrop-blur-xl border-b border-slate-200/90 px-6 py-6 space-y-4 shadow-xl">
            ...
            <div className="pt-3 border-t border-slate-100">
              <Link
                href="/dao"
                onClick={(e) => { setMobileMenuOpen(false); handleLaunchApp(e); }}
